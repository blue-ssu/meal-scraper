...
export class SoongguriScraper implements MenuScraper {
  constructor(
    ...
    private readonly restaurantType: RestaurantType,
  ) {}
  ...
  async scrapeMenu(date: string) {
    ...
    if (hasHoliday) {
      throw new HolidayException(date, this.restaurantType, '해당일은 휴무일입니다.', html);
    }
    ...
    if (!Object.keys(menus).length) {
      throw new MenuFetchException(date, this.restaurantType, '메뉴를 찾지 못했습니다.', { menus, html });
    }

    return {
      date,
      restaurant: this.restaurantType,
      menuTexts: menus,
    };
  }
}
